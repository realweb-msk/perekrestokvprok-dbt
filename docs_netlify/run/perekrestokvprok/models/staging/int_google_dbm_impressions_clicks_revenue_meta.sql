  ...
  create or replace view `perekrestokvprok-bq`.`dbt_lazuta`.`int_google_dbm_impressions_clicks_revenue_meta`
  OPTIONS()
...
new_source AS (
    ...
    FROM `perekrestokvprok-bq`.`dbt_lazuta`.`stg_google_dbm`
    WHERE date >= '2021-12-01'
),
