  ...
  create or replace view `perekrestokvprok-bq`.`dbt_krepin`.`dim_ua`
  OPTIONS()
  as /* 
...
mt_beta_cost AS (
 ...
 as promo_search,
        ...
        0 clicks,
        cost AS spend
    FROM `perekrestokvprok-bq`.`dbt_krepin`.`stg_vk_beta_sheet`
    WHERE campaign_type = 'UA'
),
...
asa_cost AS (
 ...
 as promo_search,
    ...
    --`perekrestokvprok-bq`.`dbt_krepin`.`int_asa_cab_meta`
...
huawei_cost AS (
 ...
 as promo_search,
        ...
        SUM(clicks) AS clicks,
        SUM(spend) AS spend
    FROM `perekrestokvprok-bq`.`dbt_krepin`.`stg_huawei_cab_sheets`
    WHERE campaign_type = 'UA'
    AND status != "Deleted"
    GROUP BY 1,2,3,4,5,6,7
),
...
zen_cost AS (
 ...
 as promo_search,
        ...
        0 clicks,
        SUM(cost) AS spend
    FROM `perekrestokvprok-bq`.`dbt_krepin`.`stg_zen_data_sheets`
    WHERE campaign_type = 'UA'
    GROUP BY 1,2,3,4,5,6,7
),
...
campaign_limits AS (
    SELECT
        ...
        limits
    FROM `perekrestokvprok-bq`.`dbt_krepin`.`stg_campaign_limits`
),
...
inapp_orders AS (
    SELECT
        ...
        campaign,
        orders
    FROM `perekrestokvprok-bq`.`dbt_krepin`.`stg_crm_orders`
),
...
realwebcpa_orders AS (
    SELECT
        ...
        campaign,
        orders
    FROM `perekrestokvprok-bq`.`dbt_krepin`.`stg_crm_orders_realwebcpa`
),
